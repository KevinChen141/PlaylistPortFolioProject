...
- [y] I have drafted my third component idea in the "Component Designs" section
...
Some of my interests are computer science, sports (basketball, pickleball, and
more), music, and food. My career goal is to become a software developer. I
really enjoy playing sports and I use to play basketball for the school team. I
recently started to get into pickleball because my friends introduced it to me.
I enjoy listening to music and like to cook.
...
- Component Design #1: `Music Playlist`
  ...
  - **Description**: The purpose of this component is to model a music playlist.
    The user would be able to organize a set of songs and put them together in
    a playlist. The kernal methods would serve to add and remove songs to the
    playlist. Secondary methods could include shuffling the playlist, sort the
    songs in the playlist, etc.
  - **Kernel Methods**:
    `add()`: adds a song to this
    `remove(x)`: removes a song `x` from this and returns it
    `length()`: returns the length of this
  - **Secondary Methods**:
    `sort(Comparator<T> order)`: sorts the songs in this in order
    `shuffle()`: sorts the songs in the playlist in a random order

- Component Design #2: `To-do list/recipe list for cooking`

  - **Description**: The purpose of this component is to model a to-do list or
    recipe list for cooking. User would be able to add tasks/recipes and their
    respective time/amount. Then they could add mini tasks for the main tasks.
    The main task would be marked as complete if the mini tasks for the main task
    is complete. The whole list is complete when all main tasks are complete. The
    kernal methods would include add, remove, length, markComplete, markIncomplete,
    etc. Secondary methods could include isComplete, toString, nextTask, etc.
  - **Kernel Methods**:
    `add(String task/ingredient, int time/amount)`: adds a task to this
    `addTo(String mainTask, int time/amount, String task)`: adds a mini task to
    `mainTask`
    `remove(String x)`: removes a task `x` from this and returns it. If `x` had
    mini tasks under it, then those will also be removed.
    `length()`: returns the length of this
    `markComplete(String task)`: mark `task` as complete
    `markIncomplete(String task)`: mark `task` as incomplete
  - **Secondary Methods**:
    `isComplete()`: returns a boolean of if all tasks are complete or not
    `toString()`: returns the list in string format
    `nextTask()`: returns the next task that is incomplete

- Component Design #3: `Basketball stats tracker`
  - **Description**: The purpose of this component is to model a stats tracker
    for a basketball team. User would be able to create a team, create players,
    and keep track of stats for players and the team. Kernal methods could include
    addPlayer(), addStat(), removePlayer(), removeStat(), etc. Secondary methods
    could include toString(), getStat(), etc.
  - **Kernel Methods**:
    `addPlayer(String name, int number)`: adds a player to this
    `removePlayer(String name)`: removes a player from this an returns it
    `addStat(String stat, int number, String player)`: adds a stat to the player
    `removeStat(String stat, int number, String player)`: removes a stat from the
    player
  - **Secondary Methods**:
    `toString()`: returns the team, players, and stats in a string
    `getStat(String stat)`: returns the integer value for a stat
